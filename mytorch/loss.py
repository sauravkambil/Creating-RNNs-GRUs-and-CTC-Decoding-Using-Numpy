...
class SoftmaxCrossEntropy(Criterion):
    ...
    def forward(self, x, y):
        ...
        for i in range(C):
            self.softmax = np.exp(x)/np.sum(np.exp(x),axis=1, keepdims=True)  # TODO
        crossentropy = np.matmul(np.multiply(-y,np.log(self.softmax)),Ones_C)  # TODO
        sum_crossentropy = np.matmul(np.transpose(Ones_N),crossentropy)  # TODO
